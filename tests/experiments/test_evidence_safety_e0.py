...
def test_repaired_c1_rejects_mixed_valid_plus_ambiguous_same_scope(evaluation):
    for policy_id in PolicyID:
        trial = _trial(
            evaluation,
...
def test_repaired_stack_summary_has_zero_acr_and_zero_sser_per_policy(evaluation):
    for policy_id in PolicyID:
        ...
        assert (acr.numerator, acr.denominator) == (0, 9)
        assert (sser.numerator, sser.denominator) == (0, 9)
        ...
        assert _outcome_count(summary, OutcomeClass.O3_EXPLICIT_NON_SUCCESS) == 9
