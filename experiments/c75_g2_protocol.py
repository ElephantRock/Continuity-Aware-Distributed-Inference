...
C75_AUGMENTATION_SCHEMA = "cadi.c7.5a.trace-augmentation.v1"
...
def seed_selection_fingerprint(
    ...
    if admissible_dataset_fingerprint != C75_C72_ADMISSIBLE_DATASET_FINGERPRINT:
        raise ValueError("admissible dataset fingerprint drift")
    selected = select_seed_records(records)
    payload = {
        "admissible_fingerprint": admissible_dataset_fingerprint,
        "selected_count": len(selected),
        "record_ids": [record.record_id for record in selected],
        "input_tokens": [record.input_tokens for record in selected],
        "output_tokens": [record.output_tokens for record in selected],
    }
...
def _validated_worker_count(worker_count: int) -> int:
    if not isinstance(worker_count, int) or isinstance(worker_count, bool):
        raise TypeError("worker_count must be int")
    if worker_count not in C75_WORKER_COUNTS:
        raise ValueError("worker_count must be a frozen C7.5 worker-count value")
    return worker_count
...
def state_worker_index(seed: int, worker_count: int, *, operation_ordinal: int = 0) -> int:
    ...
    return _seed_fact(seed, f"state-location:{operation_ordinal}", workers)


def session_preferred_worker_index(seed: int, worker_count: int) -> int:
    workers = _validated_worker_count(worker_count)
    return _seed_fact(seed, "session-preferred", workers)


def worker_queue_depth(
    seed: int, worker_index: int, worker_count: int, *, operation_ordinal: int = 0
) -> int:
    workers = _validated_worker_count(worker_count)
    if not isinstance(worker_index, int) or isinstance(worker_index, bool) or not 0 <= worker_index < workers:
        raise ValueError("worker_index must identify a worker in the frozen realization")
    return _seed_fact(seed, f"worker-load:{operation_ordinal}:{worker_index}", C75_QUEUE_LEVELS)


def p4_state_worker_index(seed: int) -> int:
    return _seed_fact(seed, "p4-shared-state-location", C75_P4_WORKER_COUNT)
...
def p4_worker_queue_depth(seed: int, worker_index: int, *, branch_ordinal: int = 0) -> int:
    if (
        ...
        or not 0 <= worker_index < C75_P4_WORKER_COUNT
    ):
        raise ValueError("worker_index must identify a P4 reference worker")
...
def augmentation_fingerprint(
    *,
    ...
    if seed not in C7_STOCHASTIC_SEEDS:
        raise ValueError("seed must come from frozen C7.1 sequence 0..63")
    if series not in C75_AUGMENTATION_PARAMETERS:
        raise ValueError("series is outside the C7.5a P1/P4/P7 execution set")
    if not isinstance(source_record_id, str) or not source_record_id:
        raise ValueError("source_record_id must be non-empty")
    _sha256_hex(program_case_fingerprint, "program_case_fingerprint")
    if not isinstance(parameters, tuple):
        raise TypeError("parameters must be tuple")
    names = tuple(name for name, _ in parameters)
    if names != tuple(sorted(names)) or len(names) != len(set(names)):
        raise ValueError("parameters must have unique canonical names")
    if names != C75_AUGMENTATION_PARAMETERS[series]:
        raise ValueError("parameters do not exactly match the frozen C7.5 series contract")
    for name, value in parameters:
        axis = AXES[name]
        if value not in axis.values:
            raise ValueError(f"parameter {name} escaped the frozen C7.1 axis")
    values = dict(parameters)
    if series is ExperimentSeries.P4_FANOUT_SHARED_PREFIX and values["worker_count"] != C75_P4_WORKER_COUNT:
        raise ValueError("P4 worker_count must remain at the frozen reference value")
    if series is ExperimentSeries.P7_STATELESS_OVERHEAD and values["arrival_intensity"] != C75_P7_ARRIVAL_INTENSITY:
        raise ValueError("P7 arrival_intensity must remain at the frozen nominal control value")
    return _fingerprint(
        {
            "schema": C75_AUGMENTATION_SCHEMA,
            "seed": seed,
            "series": series.value,
            "source_record_id": source_record_id,
            "program_case_fingerprint": program_case_fingerprint,
            "parameters": {name: value for name, value in parameters},
        }
    )
...
def adjudicate_g2(
    *, h4_supported: bool, h5_supported: bool, h6_supported: bool, h7_supported: bool
) -> C75G2Decision:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "workload_realization": {
                ...
                "selection_pool": (
                    "C7-admissible Mooncake rows with input_tokens divisible by 4; "
                ...
                "independent_seed_domains": [
                    "worker queue depth",
                    "physical State location",
                    "session preferred location",
                ],
                "augmentation_schema": C75_AUGMENTATION_SCHEMA,
                "augmentation_fingerprint": (
                    "SHA256 of canonical {schema,seed,series,source_record_id,"
                    "program_case_fingerprint,parameters}; stored in C7ExperimentManifest.augmentation_fingerprint"
                ),
                "p1": (
                    "one selected source token pair per Program; root zero reuse; later operations use exact "
                    "frozen reusable-prefix fraction"
                ),
                "p4": (
                    "one selected source token pair per Program replicated across synthetic fan-out branches; "
                    "shared State has one seed-derived physical location; branch load facts are branch-specific"
                ),
                "p7": (
                    "one source-shaped stateless request with no cross-request State/session/Continuity locality hints"
                ),
            },
            ...
            "representability_limits": {
                ...
                "rule": (
                    "unrealized axes may be reported as limitations but may not enter ranking or support claims"
                ),
            },
            "h4": {
                ...
                "cell_support": (
                    "B4 and comparator semantically eligible; paired Program-cluster percentile-95 CI for "
                    "RR(B4)-RR(baseline) entirely <0; RR identical across accepted hardware strata"
                ),
                "aggregate_estimator": "ratio-of-sums over Program token components",
                "minimum_connected_cells": C75_MIN_CONNECTED_SUPPORT_CELLS,
                "region": (
                    "orthogonal adjacency within one fixed-worker-count P1 surface for the same comparator"
                ),
                "hypothesis_support": (
                    "at least one primary comparator has a qualifying connected region; all B0/B1/B2 maps "
                    "and no-benefit regions must be reported"
                ),
                "cache_aware_confluence_note": (
                    "C7.2 may make B1 and B4 coincide when the same compatible resident State locality is "
                    "visible; a B1 tie is a valid no-benefit result, not grounds to weaken B1"
                ),
                "correlated_explanatory_metrics": [
                    MetricID.STATE_REUSE_RATIO.value,
                    MetricID.STATE_REUSE_TOKEN_RATIO.value,
                    MetricID.COLD_CONTINUATION_RATE.value,
                ],
            },
            "h7": {
                ...
                "gradient": (
                    "axis-aligned contiguous run at fixed other-axis value (and fixed worker-count stratum "
                    "for P1); >=3 ordered levels; each cell percentile-95 benefit CI entirely >0; median "
                    "benefit nondecreasing with >=1 strict increase; same direction on both hardware strata"
                ),
                "minimum_ordered_levels": C75_H7_MIN_ORDERED_LEVELS,
                "hypothesis_support": (
                    "at least one qualifying P1 or P4 statefulness gradient plus passing P7 negative control"
                ),
                "multi_surface_confirmation": (
                    "qualifying gradients in both P1 and P4 are reported as stronger cross-dimension "
                    "confirmation but are not required by H7"
                ),
                "interaction_rule": (
                    "non-qualifying and non-monotone axes remain reportable interactions; H7 does not "
                    "require monotonicity for every variable"
                ),
            },
            "p7_negative_control": {
            ...
            "ablation_boundary": {
                "b0_b4_role": (
                    "information-contract decomposition; not pure single-mechanism causal ablation"
                ),
                "b4_vs_b3": (
                    "diagnostic only unless separately supported by closed targeted evidence"
                ),
                "closed_h5_h6_recomputed": False,
            },
            "gate_g2": {
                "A": (
                    "H7 supported and at least two of H4/H5/H6 supported or strengthened within their "
                ...
                "evaluation_order": [
                    C75G2Decision.A.value,
                    C75G2Decision.C.value,
                    C75G2Decision.B.value,
                ],
            },
...
def main() -> None:
    print(
        _canonical_json(
            {
                "protocol": FROZEN_C75_PROTOCOL.to_dict(),
                "protocol_fingerprint": C75_PROTOCOL_FINGERPRINT,
            }
        )
    )
