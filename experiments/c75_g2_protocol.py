...
class C75H4Decision(str, Enum):
    SUPPORTED = "SUPPORTED_WITHIN_DECLARED_PHASE_SPACE"
    NOT_SUPPORTED = "NOT_SUPPORTED_WITHIN_DECLARED_PHASE_SPACE"


class C75H7Decision(str, Enum):
    SUPPORTED = "SUPPORTED_WITHIN_DECLARED_PHASE_SPACE"
    NOT_SUPPORTED = "NOT_SUPPORTED_WITHIN_DECLARED_PHASE_SPACE"
...
def reusable_tokens(input_tokens: int, fraction: float) -> int:
    if not source_record_eligible(input_tokens, 1):
        raise ValueError("input_tokens must satisfy the C7.5 source-selection arithmetic constraint")
...
def _seed_fact(seed: int, label: str, modulo: int) -> int:
    ...
    digest = hashlib.sha256(
        f"{C75_RESOURCE_SALT}|{label}|{seed}".encode("utf-8")
    ).digest()
...
def p4_worker_queue_depth(seed: int, worker_index: int) -> int:
    if not isinstance(worker_index, int) or isinstance(worker_index, bool) or worker_index < 0:
        raise ValueError("worker_index must be non-negative integer")
    return _seed_fact(seed, f"p4-worker-load:{worker_index}", C75_QUEUE_LEVELS)
...
class C75Protocol:
    ...
    def __post_init__(self) -> None:
        ...
        if C7_PROTOCOL_FINGERPRINT != "706e0d5fff362a1eda8c906b957c914251c6e7949bac6be3c2c143ae21625474":
            raise RuntimeError("C7.1 protocol drift")
        if C72_BASE_COMMIT != "990ef4f081b0edebb1e2ca83bddaf252eb39e549":
            raise RuntimeError("C7.2 harness parent drift")
        if C6_SCIENTIFIC_FINGERPRINT != "cc1b62c4e38a1612720f601375c425c0bd7d67b4eff11b101bfdf87b79a7a61a":
            raise RuntimeError("C6 scientific fingerprint drift")
        if C6_ARTIFACT_SHA256 != "93990386135ecbf6fc38e579841eace3431a068b10ee59209fe6ad7084bd8889":
            raise RuntimeError("C6 artifact drift")
        if C6_EVIDENCE_CLASS != "SIMULATED_SOURCE_MODEL_DERIVED_P_SRC2":
            raise RuntimeError("C6 evidence class drift")
        ...
        if WorkloadClass.TRACE_AUGMENTED.value != "TRACE-AUGMENTED":
            raise RuntimeError("TRACE-AUGMENTED identity drift")
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "workload_realization": {
                ...
                "selection_pool": (
                    "C7-admissible Mooncake records with input_tokens divisible by 4; "
                    "no source-field mutation"
                ),
                "selection_order": "ascending SHA256(source_selection_salt|record_id)",
                ...
                "p1": (
                    "one selected source token pair per Program; depth is synthetic continuation structure; "
                    "root has zero reuse; later operations use exact frozen reusable-prefix fraction"
                ),
                "p4": (
                    "one selected source token pair per Program replicated across synthetic fan-out branches; "
                    "all branches share one independently selected physical State location"
                ),
                "p7": "one source-shaped stateless request; no State/session/Continuity locality hints",
            },
            ...
            "h4": {
                ...
                "cell_support": (
                    "B0/B1/B2/B4 semantically eligible; paired Program-cluster percentile-95 "
                    "CI for RR(B4)-RR(Bj) entirely <0 for each primary comparator; "
                    "non-timing RR identical across accepted hardware strata"
                ),
                "aggregate_estimator": "ratio-of-sums over Program token components",
                "minimum_connected_cells": C75_MIN_CONNECTED_SUPPORT_CELLS,
                "region": "orthogonal adjacency within one fixed-worker-count P1 surface",
            ...
            "h7": {
                ...
                "gradient": (
                    "axis-aligned contiguous run at fixed other-axis value; at least 3 ordered levels; "
                    "all cell percentile-95 benefit CIs entirely >0; median benefit nondecreasing "
                    "with at least one strict increase; same direction on both hardware strata"
                ),
                "minimum_ordered_levels": C75_H7_MIN_ORDERED_LEVELS,
                "support": "qualifying gradient on both P1 and P4 plus passing P7 negative control",
            },
            "p7_negative_control": {
            ...
            "gate_g2": {
                "A": (
                    "H7 supported and at least two of H4/H5/H6 supported or strengthened within "
                    "their declared phase spaces"
                ),
            ...
            "invalid_result_conditions": [
                ...
                "seed source selection missing, duplicated, or outside the declared arithmetic pool",
                "policy information-contract leak",
                "semantic-invalid observation counted as support",
                "accepted C6 hardware stratum missing",
                "timing benefit denominator is zero or non-finite",
