...
def source_request_c6_admissible(input_tokens: int, output_tokens: int) -> bool:
    """C7 source-row eligibility for the finite C6 EV3 request domain.

    Values are never clipped or truncated. Source-backed C7 timing uses a row only
    when its original source-observed token counts satisfy this predicate.
    """
...
def validated_transfer_state_admissible(state_tokens: int) -> bool:
    ...
    return state_tokens <= C6_MAX_UNSPLIT_TRANSFER_STATE_TOKENS
...
METRIC_DEFINITIONS: Mapping[MetricID, MetricDefinition] = {
    MetricID.RECOMPUTATION_RATIO: MetricDefinition(
        ...
        "sum of input tokens across the same executed inference operations",
        "fraction",
        "Program",
    ),
...
def recomputation_ratio(
    *,
    ...
    if total == 0:
        return 0.0
    return (eligible - consumed) / total
...
def residency_ratios(*, useful_byte_seconds: float, wasted_byte_seconds: float) -> tuple[float, float]:
    ...
    if total == 0.0:
        return (0.0, 0.0)
    return (useful / total, wasted / total)
...
def efficiency_eligibility(*, covered_semantic_violations: int) -> EfficiencyEligibility:
    ...
    if violations:
        return EfficiencyEligibility.SEMANTICALLY_INVALID_FOR_EFFICIENCY_RANKING
    return EfficiencyEligibility.ELIGIBLE
...
AXES: Mapping[str, AxisDefinition] = {
    "session_depth": AxisDefinition(
        "session_depth", (2, 4, 8, 16, 32), ParameterSource.P_SRC4, 8
    ),
    "reusable_prefix_fraction": AxisDefinition(
        "reusable_prefix_fraction",
        (0.0, 0.25, 0.5, 0.75, 1.0),
        ParameterSource.P_SRC4,
        0.5,
    ),
    ...
    "worker_count": AxisDefinition(
        "worker_count", (2, 4, 8), ParameterSource.P_SRC4, 4
    ),
    ...
    "branch_width": AxisDefinition(
        "branch_width", (2, 4, 8, 16), ParameterSource.P_SRC4, 4
    ),
    "speculative_fraction": AxisDefinition(
        "speculative_fraction", (0.0, 0.25, 0.5, 0.75), ParameterSource.P_SRC4, 0.25
    ),
    "fanout_width": AxisDefinition(
        "fanout_width", (2, 4, 8, 16), ParameterSource.P_SRC4, 4
    ),
    "shared_prefix_fraction": AxisDefinition(
        "shared_prefix_fraction", (0.25, 0.5, 0.75, 1.0), ParameterSource.P_SRC4, 0.5
    ),
    "state_tokens": AxisDefinition(
        "state_tokens", (1, 4, 16, 64), ParameterSource.P_SRC2, 16
    ),
    "recompute_tokens": AxisDefinition(
        "recompute_tokens", (64, 256, 1024, 2048, 4096), ParameterSource.P_SRC2, 1024
    ),
}
...
SURFACES: tuple[SurfaceDefinition, ...] = (
    SurfaceDefinition(
        ExperimentSeries.P1_DEEP_REUSE, "session_depth", "reusable_prefix_fraction"
    ),
    SurfaceDefinition(
...
class C7ExperimentManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.workload_class is WorkloadClass.REAL_TRACE and self.seed is not None:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "parameter_sources": {
                name: source.value for name, source in self.parameter_sources
            },
        }
...
class EfficiencyProtocol:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "metrics": [
                METRIC_DEFINITIONS[metric].to_dict() for metric in MetricID
            ],
...
def main() -> None:
    payload = {
        ...
        "protocol_fingerprint": FROZEN_C7_PROTOCOL.fingerprint,
    }
